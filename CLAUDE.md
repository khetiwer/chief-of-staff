...
- **Alfred acts THROUGH the CRM, never around it.** v3 FILE-CANONICAL (cutover completed 2026-08-31, contract: `crm\SPEC.md` v3.0): records live in ONE store - the pipeline files at `C:\Users\kheti\workspaces\chosen-advisory\crm\pipeline\<slug>.md`. Every record write Alfred makes goes there, via direct file edits. **Notion is retired entirely - never read it, never write it, for any CRM purpose** (final snapshot archived at `crm\state\notion-archive\`; the old Notion scripts are parked in `crm\scripts\archive\` and must not be run). Read state at `crm\state\queue.md` and `crm\state\pipeline-summary.md`. Operating skill: `advisory-crm-daily`.
